...
export const compile: (program: Program) => Runtime<ES1Value>
  = program => context => isolate<ES1Value>(some(context.currentScope), escape => compose(program.sourceElements.reduce((prev, sourceElement) => compose(prev, compileSourceElement(sourceElement, escape)), empty<ES1Value>()), escape(none)))(context);